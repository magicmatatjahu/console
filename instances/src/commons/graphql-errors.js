...
export function isServiceCatalogModuleDisabled(errors) {
  return checkErrors(errors, errorMessage =>
    isModuleDisabledErrorMessage(errorMessage, SERVICE_CATALOG),
  );
}

export function isServiceCatalogAddonsModuleDisabled(errors) {
  return checkErrors(errors, errorMessage =>
    isModuleDisabledErrorMessage(errorMessage, SERVICE_CATALOG_ADDONS),
  );
}

export function isContentModuleDisabled(errors) {
  return checkErrors(errors, errorMessage =>
    isModuleDisabledErrorMessage(errorMessage, CONTENT),
  );
}
...
function isModuleDisabledErrorMessage(errorMessage, moduleType) {
  return (
    errorMessage === `MODULE_DISABLED: The ${moduleType} module is disabled.`
  );
}
