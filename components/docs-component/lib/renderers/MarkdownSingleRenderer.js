...
const Renderer = ({ source, k = '' }) => {
  ...
  const id = toKebabCase(`${helpers_1.headingPrefix(source)}-${title}`);
  type = type || title;
  const kebabCasedType = toKebabCase(type);
  return react_1.default.createElement(
    StyledMarkdown,
    { key: k, id: kebabCasedType },
    title && react_1.default.createElement(Header, { id: id }, title),
    renderedContent,
  );
};
exports.MarkdownSingleRenderer = {
