...
describe('<Pagination />', () => {
  ...
  const itemsPerPagePagination = (
    <Pagination itemsPerPage={25} itemsTotal={101} onClick={handleClick} />
  );

  const hideTotalItemsPagination = (
    <Pagination displayTotal={false} itemsTotal={101} onClick={handleClick} />
  );
  ...
  const itemsPerPageZeroPagination = (
    <Pagination itemsPerPage={0} itemsTotal={101} onClick={handleClick} />
  );
  ...
  describe('Prop spreading', () => {
    test('should allow props to be spread to the Pagination component', () => {
      ...
      expect(
        element.find('span').getDOMNode().attributes['data-sample'].value,
      ).toBe('Sample2');
    });
    ...
    test("should allow props to be spread to the Pagination component's previous a element", () => {
      ...
      expect(
        element.find('a[aria-label="Previous"]').getDOMNode().attributes[
          'data-sample'
        ].value,
      ).toBe('Prev Sample');
    });
