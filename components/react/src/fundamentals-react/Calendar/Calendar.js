...
export class Calendar extends Component {
  ...
  static getDerivedStateFromProps(updatedPropsParent, previousStates) {
    ...
    if (typeof updatedPropsParent.enableRangeSelection !== 'undefined') {
      if (updatedPropsParent.customDate !== previousStates.arrSelectedDates) {
        ...
        return {
          arrSelectedDates: updatedPropsParent.customDate,
          selectedDate: new Date(0, 0, 0),
        };
      }
    } else if (
      updatedPropsParent.customDate !== previousStates.currentDateDisplayed
    ) {
      if (
        typeof updatedPropsParent.customDate === 'undefined' ||
        updatedPropsParent.customDate === ''
      ) {
        return null;
      } else if (updatedPropsParent.customDate === 'undefined') {
        if (
          previousStates.selectedDate.getFullYear() !== 1899 &&
          previousStates.dateClick
        ) {
          return { dateClick: false };
        }
        ...
        //Updates the calendar if the date from date picker is correct
        return {
          currentDateDisplayed: updatedPropsParent.customDate,
          selectedDate: updatedPropsParent.customDate,
          dateClick: false,
        };
      }
    }
  ...
  displaySelectedRangeLast = day => {
    return (
      this.props.enableRangeSelection &&
      (typeof this.state.arrSelectedDates[1] !== 'undefined' &&
        this.state.arrSelectedDates[1].getTime() === day.getTime())
    );
  };
  ...
  displayIsSelected = day => {
    return (this.isSelected(day) ||
      (this.props.enableRangeSelection &&
        ((typeof this.state.arrSelectedDates[0] !== 'undefined'
          ? this.state.arrSelectedDates[0].getTime() === day.getTime()
          : false) ||
          (typeof this.state.arrSelectedDates[1] !== 'undefined'
            ? this.state.arrSelectedDates[1].getTime() === day.getTime()
            : false)))) &&
  ...
  generateMonths = monthProps => {
    ...
    let listOfMonths = months.map(element => {
      ...
      let calendarItemClasses = classnames('fd-calendar__item', {
        'is-selected':
          months[this.state.currentDateDisplayed.getMonth()] === element,
        'fd-calendar__item--current':
          months[this.state.todayDate.getMonth()] === element,
      });
  ...
  generateYears = yearListProps => {
    ...
    let listOfYears = years.map(element => {
      let yearClasses = classnames('fd-calendar__item', {
        'is-selected':
          this.state.currentDateDisplayed.getFullYear() === element,
        'fd-calendar__item--current':
          this.state.todayDate.getFullYear() === element,
      });
  ...
  dateClick = (day, isRangeEnabled) => {
    ...
    if (typeof isRangeEnabled !== 'undefined' && isRangeEnabled) {
      ...
      if (selectedDates.length === 2) {
        ...
        day.getTime() <= selectedDates[0].getTime()
      ) {
        let newArr = [];
        newArr.push(day);
  ...
  isDateBetween = (date, blockedDates, isRangeEnabled) => {
    if (
      typeof blockedDates === 'undefined' ||
      typeof blockedDates[0] === 'undefined' ||
      typeof blockedDates[1] === 'undefined'
    ) {
      return false;
    }
    ...
    if (typeof isRangeEnabled !== 'undefined' || isRangeEnabled) {
      if (blockedDates[0].getTime() > blockedDates[1].getTime()) {
        return (
          blockedDates[1].getTime() < date.getTime() &&
          blockedDates[0].getTime() > date.getTime()
        );
      }
    }
    return (
  ...
  isSelected = date => {
    return (
      this.state.selectedDate.getDate() === date.getDate() &&
      ...
      <header className="fd-calendar__header">
        <div className="fd-calendar__navigation">
          <div className="fd-calendar__action">
            <button
              className="fd-button--standard fd-button--light fd-button--compact sap-icon--slim-arrow-left"
  ...
  generateDays = tableBodyProps => {
    ...
    let firstDayMonth = this.returnFirstDayMonth(
      this.state.currentDateDisplayed,
    );
    ...
    while (day <= lastDateEndMonth) {
      ...
      for (let iterations = 0; iterations < 7; iterations++) {
        ...
        let dayClasses = classnames({
          ...
          'fd-calendar__item--current':
            this.state.todayDate.getTime() === copyDate.getTime(),
    ...
    return (
      <tbody {...tableBodyProps} className="fd-calendar__group">
        {rows}
      </tbody>
    );
  };
  ...
  render() {
    ...
    const calendarClasses = classnames('fd-calendar', className);
    return (
