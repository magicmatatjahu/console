...
describe('<Table />', () => {
  ...
  const simpleTable = (
    <Table headers={defaultHeaders} tableData={defaultData} />
  );

  const simpleTableWithClass = (
    <Table className="blue" headers={defaultHeaders} tableData={defaultData} />
  );
  ...
  describe('Prop spreading', () => {
    ...
    test("should allow props to be spread to the Table component's tbody > tr elements via object", () => {
      ...
      expect(rows.at(0).getDOMNode().attributes['data-sample'].value).toBe(
        'Sample',
      );
      expect(rows.at(1).getDOMNode().attributes['data-sample'].value).toBe(
        'Sample',
      );
    });
