...
<Token>Filter</Token>`;
  ...
  return (
    <div>
      <Header>Token</Header>
      <Description>
        Tokens are used to represent contextualizing information. They can be
        useful to show applied filters, selected values for form field or object
        metadata.
      </Description>
      ...
      <DocsTile centered>
        <div className="fd-doc__margin--token">
          {tagsNames.map(name => {
            return (
              <Token clickHandler={e => closeAction(name, e)}>{name}</Token>
            );
          })}
        </div>
      </DocsTile>
