...
describe('<Modal />', () => {
  ...
  const infoNoShowModal = (
    <Modal onClose={mockOnClose} show={!bShow} title={modalInfoTitle}>
      <div>
        <b>The new product have been added to your catalog.</b>
        <br />
        <br />
  ...
  describe('Prop spreading', () => {
    test('should allow props to be spread to the Modal component', () => {
      component = mount(<Modal data-sample="Sample" show title="Title" />);
    ...
    test("should allow props to be spread to the Modal component's h1 element", () => {
      ...
      expect(
        component.find('h1').getDOMNode().attributes['data-sample'].value,
      ).toBe('Sample Title');
    });
    ...
    test("should allow props to be spread to the Modal component's footer section", () => {
      ...
      expect(
        component.find('footer').getDOMNode().attributes['data-sample'].value,
      ).toBe('Sample Title');
    });
    ...
    test("should allow props to be spread to the Modal component's button element", () => {
      ...
      expect(
        component.find('button').getDOMNode().attributes['data-sample'].value,
      ).toBe('Sample Button');
    });
  });
});
