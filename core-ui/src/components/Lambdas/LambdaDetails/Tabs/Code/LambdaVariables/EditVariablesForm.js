...
export default function EditVariablesForm({
  ...
  useEffect(() => {
    onUpdateValidate();
  }, [setValidity, variables, customVariables]);
  ...
  function onUpdateValidate() {
    ...
    // check if the custom variables have changed
    // const equal = deepEqual(variables, customVariables);
    // console.log(equal)
    // if (equal) {
    //   setValidity(false);
    //   setInvalidModalPopupMessage(ENVIRONMENT_VARIABLES_PANEL.EDIT_MODAL.CONFIRM_BUTTON.POPUP_MESSAGES
    //     .COLLECTIONS_EQUAL);
    //   return;
    // }
