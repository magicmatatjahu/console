...
const TableContainer: React.FunctionComponent = () => {
  const { addonsConfigurations, error, loading = true } = useContext(QueriesService.Context);
  const { configurationsExist, filteredConfigs } = useContext(ConfigurationsService.Context);
  ...
  const content = () => {
    ...
    if (!(filteredConfigs && filteredConfigs.length)) {
      return (
        <ErrorWrapper>
          {ERRORS.NOT_MATCHING_FILTERS}
        </ErrorWrapper>
      )
    }
    return <TableContentComponent headers={CONTENT_HEADERS} configs={filteredConfigs} setFilterLabel={setFilterLabel} />;
  }
